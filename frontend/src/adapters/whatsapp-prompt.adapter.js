...
export const whatsappPromptsAdapter = (prompts) => {
  const adaptedPrompts = [];
  if (!prompts || prompts.length === 0) return adaptedPrompts;

  prompts.forEach((prompt) => {
    adaptedPrompts.push(whatsappPromptAdapter(prompt));
  });

  return adaptedPrompts;
};

export const whatsappPromptAdapter = (prompt) => ({
  name: prompt.name,
  id: prompt._id,
...
export const whatsappPromptEditAdapter = (prompt) => ({
  id: prompt?._id,
  name: prompt?.name,
...
export const createWhatsappPromptAdapter = (data) => ({
