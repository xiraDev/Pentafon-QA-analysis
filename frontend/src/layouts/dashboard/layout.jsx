...
export function DashboardLayout({ sx, children, data }) {
  ...
  // Esta función entra y obtiene sólo los datos que permitan el rol actual
  const filterNavigationByRole = (navData, role) => {
    const filteredNavigation = navData.map(n => (
        {
            ...n,
            items: n.items.map(ni=>(
              {
                ...ni, 
                ...(ni.children && 
                  {
                  children: ni.children.filter(i => i.roles ? i.roles.includes(role) : true)
                  }
                )
              }
            )
    ...
    return filteredNavigation.filter(n => n.restricted ? !n.restricted.includes(role) : true)
  }
  ...
  return (
    <>
      <NavMobile
        data={filterNavigationByRole(navData, currentRole)}
