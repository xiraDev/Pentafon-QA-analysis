...
export const NavItem = forwardRef(({ title, path, icon, open, active, hasChild, externalLink, ...other }, ref) => {
  ...
  return (
    <StyledNavItem ref={ref} aria-label={title} open={open} active={active} {...navItem.baseProps} {...other}>
      {navItem.renderIcon}
      ...
      {hasChild && <Iconify width={16} icon={open ? 'eva:arrow-ios-downward-fill' : 'eva:arrow-ios-forward-fill'} />}
    </StyledNavItem>
  );
});
